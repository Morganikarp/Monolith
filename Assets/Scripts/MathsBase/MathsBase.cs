...
public class Vect3
{
    ...
    public static Vect3 EulerAnglesToDirVect(Vect3 Vect, bool ifDegree) //Converts a Vect3 euler angle into a Vect3 directional vector (converts degree angles to radians before processing)
    {
        Vect3 result = Zero;

        Vect3 adjustedVect = Vect;

        if (ifDegree)
        {
            adjustedVect = ApplyScalar(Vect, (MathF.PI / 180));
        }
    ...
    public static Vect3 RotateVertextAroundAxis(float Angle, Vect3 Axis, Vect3 Vertex)
    {
        Vect3 rv = (Vertex * MathF.Cos(Angle)) + 
            Axis * DotProduct(Vertex, Axis, false) * (1 - MathF.Cos(Angle)) + 
